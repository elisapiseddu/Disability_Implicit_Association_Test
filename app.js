  ...
  trial(display, trial) {
    ...
    const touch =
      trial.response_mode === 'touch'
        ? `<button class="touch-zone left" data-key="${LEFT_KEY}">LEFT</button>
           <button class="touch-zone right" data-key="${RIGHT_KEY}">RIGHT</button>`
        : `<span class="key-label">Press E</span>
           <span class="key-label">Press I</span>`;
...
function instructionTrial(title, leftLabels, rightLabels, mode, block) {
  ...
  return {
    ...
    content: `
      ...
      <p>${keyText}</p>
      <p>
        Respond as quickly as you can while making as few mistakes as possible.
        A red X means the response was wrong; give the correct response to continue.
      </p>
      <div class="instruction-grid">
        <div class="instruction-box">LEFT<br>${leftLabels}</div>
        <div class="instruction-box">RIGHT<br>${rightLabels}</div>
      </div>
    `,
    response_mode: mode,
    data: {
...
function makeStimulusItems() {
  ...
  const good = GOOD_WORDS.map(x => ({
    id: `good_${x.toLowerCase()}`,
    kind: 'word',
    value: x,
    category: 'good'
  }));

  const bad = BAD_WORDS.map(x => ({
    id: `bad_${x.toLowerCase()}`,
    kind: 'word',
    value: x,
    category: 'bad'
  }));
...
function labelsFor(mapping, combined = false) {
  const fmt = arr =>
    arr.map(x => `<div>${x}</div>`).join('<div class="iat-or">or</div>');
...
function showFinish(session) {
  ...
  target.innerHTML = `
    <section class="finish-screen">
      ...
      <p>
        <strong>Saved locally on this tablet.</strong>
      ...
      <div class="download-row">
        <button id="save-csv">Download participant CSV again</button>
        <button id="save-json">Save JSON backup</button>
      </div>
...
async function startExperiment(metadata) {
  ...
  timeline.push({
    type: ContinuePlugin,
    content: `
      <h1>Disability Implicit Association Test</h1>
      <p>
        You will sort symbols and words into categories shown at the top-left
        and top-right of the screen.
      </p>
      <p>
        Work quickly, but try not to make mistakes. Keep your index fingers
        ready on the response keys or buttons.
      </p>
    `,
